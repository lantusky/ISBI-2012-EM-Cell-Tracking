...
def conv_BN(x, n_kernel, f_size):
    x = Conv2D(n_kernel, (f_size, f_size), strides=(1, 1), padding='same')(x)
...
def u_res_side_fuse_v2(input_shape=None):

    inputs = Input(shape=input_shape)       # 512, 512, 1
    # Normalization
    x = Lambda(lambda x: x / 255, name='pre-process')(inputs)

    # Block 1
    c1 = conv_block(x, 3, (8, 8, 32), stage=1, block='a', strides=(1, 1))
    c1 = identity_block(c1, 3, (8, 8, 32), stage=1, block='b')      # 512, 512, 32

    # Block 2
    c2 = conv_block(c1, 3, (16, 16, 64), stage=2, block='a', strides=(2, 2))
    c2 = identity_block(c2, 3, (16, 16, 64), stage=2, block='b')    # 256, 256, 64

    # Block 3
    c3 = conv_block(c2, 3, (32, 32, 128), stage=3, block='a', strides=(2, 2))
    c3 = identity_block(c3, 3, (32, 32, 128), stage=3, block='b')   # 128, 128, 128

    # Block 4
    c4 = conv_block(c3, 3, (32, 32, 128), stage=4, block='a', strides=(2, 2))
    c4 = identity_block(c4, 3, (32, 32, 128), stage=4, block='b')   # 64, 64, 128

    # Block 5
    c5 = conv_block(c4, 3, (64, 64, 128), stage=5, block='a', strides=(2, 2))
    c5 = identity_block(c5, 3, (64, 64, 128), stage=5, block='b')  # 32, 32, 256
    s1 = side_out(c5, 16)
    ...
    c6 = identity_block(c6, 3, (32, 32, 128), stage=6, block='b')   # 64, 64, 128
    s2 = side_out(c6, 8)
    ...
    c7 = conv_block(u7, 3, (32, 32, 128), stage=7, block='a', strides=(1, 1))
    c7 = identity_block(c7, 3, (32, 32, 128), stage=7, block='b')   # 128, 128, 128
    s3 = side_out(c7, 4)
    ...
    c8 = conv_block(u8, 3, (16, 16, 64), stage=8, block='a', strides=(1, 1))
    c8 = identity_block(c8, 3, (16, 16, 64), stage=8, block='b')    # 256, 256, 64
    s4 = side_out(c8, 2)
    ...
    c9 = conv_block(u9, 3, (8, 8, 32), stage=9, block='a', strides=(1, 1))
    c9 = identity_block(c9, 3, (8, 8, 32), stage=9, block='b')  # 512, 512, 32
    s5 = side_out(c9, 1)

    # fuse
    fuse = concatenate(inputs=[s1, s2, s3, s4, s5], axis=-1)
    fuse = Conv2D(1, (1, 1), padding='same', activation=None)(fuse)       # 512, 512, 1
...
def u_res_side_fuse_v3(input_shape=None):
    ...
    # Block 1
    c1 = conv_block(x, 3, (8, 8, 32), stage=1, block='a', strides=(2, 2))
    c1 = identity_block(c1, 3, (8, 8, 32), stage=1, block='b')      # 256

    # Block 2
    c2 = conv_block(c1, 3, (16, 16, 64), stage=2, block='a', strides=(2, 2))
    c2 = identity_block(c2, 3, (16, 16, 64), stage=2, block='b')    # 128

    # Block 3
    c3 = conv_block(c2, 3, (32, 32, 128), stage=3, block='a', strides=(2, 2))
    c3 = identity_block(c3, 3, (32, 32, 128), stage=3, block='b')   # 64

    # Block 4
    c4 = conv_block(c3, 3, (64, 64, 256), stage=4, block='a', strides=(2, 2))
    c4 = identity_block(c4, 3, (64, 64, 256), stage=4, block='b')   # 32

    # Block 5
    c5 = conv_block(c4, 3, (128, 128, 512), stage=5, block='a', strides=(2, 2))
    c5 = identity_block(c5, 3, (128, 128, 512), stage=5, block='b')  # 16

    # Block 6
    u6 = Conv2DTranspose(256, (2, 2), strides=(2, 2), padding='same', name='upconv_6', kernel_initializer=init)(c5)
    u6 = concatenate([u6, c4], name='concat_6')
    c6 = conv_BN(u6, 256, 3)
    c6 = conv_BN(c6, 256, 3)    # 32
    s1 = side_out(c6, 16)
